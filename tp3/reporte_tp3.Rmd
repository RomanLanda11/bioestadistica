...
Utilizamos el método indirecto para calcular la relación entre el número observado y esperado de casos, es decir las razones de mortalidad estandarizadas.
...
df_resultados <- df_resultados %>%
  mutate(
    ...
    # tools::toTitleCase convierte "del" a "Del". Si quieres "del" en minúscula,
    # puedes hacer una corrección manual, por ejemplo:
    provincia = ifelse(provincia == "Santiago Del Estero", "Santiago del Estero", provincia),
    
    # 4. Ajustar "Caba" a "C.A.B.A." o "Ciudad Autónoma de Buenos Aires" si prefieres
    provincia = ifelse(provincia == "Caba", "C.A.B.A.", provincia) # O "Ciudad Autónoma de Buenos Aires"
  ) %>%
  # Opcional: Reordenar las provincias alfabéticamente o por algún criterio si lo deseas para el gráfico
  arrange(provincia)
...
kable(tabla_para_mostrar, caption = "Tabla 1: Tasas de Mortalidad por Provincia (por 1.000 habitantes)") %>%
  kable_styling(
    ...
    position = "left") %>%  # Alinea la tabla a la izquierda
    row_spec(row = 0, background = "steelblue", color = "black", extra_css = "font-weight: bold;")
...
Lo que más llama la atención en el gráfico 1 es Misiones, ya que tiene una tasa estandarizada muy por arriba de su tasa cruda, y ambas son muy elevadas en comparación con otras provincias y la tasa de mortalidad de Argentina. Esto es una señal de alerta ya que Misiones tiene una estructura de edad que es más joven que la población de referencia (o tiene una alta proporción de población en edades de baja mortalidad), pero aún así presenta una mortalidad muy elevada. Esto sugiere que el riesgo de muerte es muy alto en Misiones y esto no se debe al envejecimiento de la población. 
...
![](mapa_tasas.jpeg)
En el mapa también vemos que la única provincia con tasa muy alta es Misiones ya que aparece en color rojo.
...
# Mostrar la tabla usando kable
kable(tabla_para_mostrar2, caption = "Tabla 2: Razones de Mortalidad Estandarizadas por Provincia") %>%
  kable_styling(
    bootstrap_options = c("striped", "hover", "condensed", "responsive"), # Opciones de estilo Bootstrap
    full_width = FALSE, # La tabla no ocupa todo el ancho de la página
    position = "left") %>%  # Alinea la tabla a la izquierda
    row_spec(row = 0, background = "steelblue", color = "black", extra_css = "font-weight: bold;") %>% 
  row_spec(row = c(2, 9, 11, 13, 14, 19, 24), background = "lightblue", color = "black")
...
Una vez calculadas las razones de mortalidad estandarizadas para cada provincia por el método indirecto vemos que 
